...
public class FileController {
	...
	private Path getPathLocation(Optional<String> path) {
		...
		return Paths
				.get(path.filter(p -> !p.isEmpty())
						.orElseThrow(() -> new InvalidPathException("path is required " + "field")))
	...
	public List<UploadFileResponse> uploadMultipleFiles(@RequestParam("files") MultipartFile[] files,
			@RequestParam String path, @RequestParam String clientName) {
		return Arrays.asList(files).stream().map(file -> uploadFile(file, path, clientName))
